...
<div class="modal-body">
    ...
    @csrf
        ...
        <div class="preview">
            ...
            <input accept="image/*" type="file" id="file-input" name="{{$conf->key}}"/>
        </div>
        @elseif($conf->type == 'textarea')
        <div class="mb-3">
            <label for="exampleFormControlTextarea1" class="form-label">{{$conf->title}}</label>
            <textarea class="form-control" id="exampleFormControlTextarea1" rows="3" name="{{$conf->key}}">{{$user->bio}}</textarea>
        </div>
        @elseif($conf->type == 'checkbox')
        <div class="mb-3">
            <div class="form-check">
                <input class="form-check-input checkbox" data-id="{{$conf->key}}" type="checkbox" value="1" id="flexCheckDefault" {{$user->enable_comment == '1' ? 'checked' : ''}}>
                <input type="hidden" id="{{$conf->key}}" value = "{{$user->enable_comment == '1' ? '1' : '0'}}" name="{{$conf->key}}">
